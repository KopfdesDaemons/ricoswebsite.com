...
// src/app/models/post-meta.model.ts
var PostMeta = class {
  constructor(fileName) {
    this.keywords = [];
    this.commentsDisabled = false;
    this.hasCodePen = false;
    this.hideInFeed = false;
    this.fileName = fileName;
  }
  initFromObject(object, lang) {
    Object.assign(this, object);
    if (object.date)
      this.date = new Date(object.date);
    if (this.date)
      this.localDateString = new Date(this.date).toLocaleDateString(lang);
  }
};

// src/app/models/post.model.ts
var Post = class {
  constructor(metaObject, fileName, lang, postContent) {
...
// src/app/helpers/markdown.helper.ts
var MarkdownHelper = class {
  static extractYamlHeader(markdown) {
    const yamlRegex = /^---([\s\S]*?)---/;
  ...
  static parseMarkdown(markdown) {
    return __async(this, null, function* () {
      ...
      return yield md.parse(markdown);
    });
  }
};
...
var PostService = class _PostService {
  ...
  getPost(fileName) {
    return __async(this, null, function* () {
      ...
      if (!this.post)
        this.post = yield this.loadFromMarkdownFile(fileName, lang);
      if (!this.post)
        return null;
  ...
  savePostTransfereState(title) {
    const key = makeStateKey("post-" + this.languageS.userLanguage + "-" + title);
    this.transferState.set(key, this.post);
  }
  loadPostFromTransfareState(title) {
  ...
  loadFromMarkdownFile(fileName, language) {
    return __async(this, null, function* () {
  ...
  loadPostList(_0) {
    return __async(this, arguments, function* (language, page = 1, pageSize = 10, sortOrder = "desc", sortBy = "date", filterTags = [], filterTitle = "") {
      const postListURL = `${this.baseUrl}posts/posts.${language}.json`;
      const json = yield lastValueFrom(this.http.get(postListURL, { responseType: "text" }));
      const posts = JSON.parse(json);
      const visiblePosts = posts.filter((post) => !post.hideInFeed && (filterTags.length === 0 || filterTags.some((keyword) => post.keywords.some((postTag) => postTag.includes(keyword)))) && (filterTitle === "" || post.title && post.title.includes(filterTitle)));
      const postArray = [];
